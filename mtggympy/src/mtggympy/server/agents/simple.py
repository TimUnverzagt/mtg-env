...
class Monkey(AgentBase):
    ...
    def decide_on_action(self, upcoming_action: PlayerEvent) -> ActionIntent:
        random_index: int = self.random_generator.randint(0, len(upcoming_action.value.possible_actions) - 1)
        random_action: ActionData = upcoming_action.value.possible_actions[random_index]
