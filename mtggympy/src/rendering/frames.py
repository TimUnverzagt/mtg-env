...
import rendering.surface_functions as SurfaceFuntions
...
def compose_surfaces_into_bounding_frame(name: str, surfaces: list[Surface], target_box: BoundingBox) -> FrameTree:
    ...
    for surface in surfaces:
        ...
        sub_frame: FrameTree = FrameTree(name + "-child", sub_frame_dimensions, sub_frame_offsets)
    ...
    containing_frame.scale_by(frame_scaling, target_box)
    return containing_frame


def build_player_ui_frame(player_box: BoundingBox, player_number:int, game_state: GameState) -> FrameTree:
    info: PlayerInfo = game_state.player_infos[player_number - 1]
    ui_target_box:BoundingBox = BoundingBox(
        Vector(player_box.dimensions[0] * 0.2, player_box.dimensions[1]), 
        player_box.offsets)
    ...
    ui_frame: FrameTree = compose_surfaces_into_bounding_frame("PlayerUI"+str(player_number),ui_texts, ui_target_box)
    return ui_frame

def render_card(card: CardInstance) -> Surface:
    file_name:str = card.card_name + '.jpg'
    surface: Surface = pygame.image.load(os.path.join(ROOT_DIR, 'assets', 'cards', file_name))
    return SurfaceFuntions.pad_surface(surface, padding_ratio=0.2)

def build_player_hand_frame(player_box: BoundingBox, player_number:int, game_state: GameState, render_topside: bool):
    info: PlayerInfo = game_state.player_infos[player_number-1]
    y_offset_ratio:float = 0 if render_topside else 0.5
    hand_target_box: BoundingBox = BoundingBox(
        Vector(player_box.dimensions[0]*0.8, player_box.dimensions[1]*0.5), 
        Vector(player_box.offsets[0] + player_box.dimensions[0]* (0.2), 
               player_box.offsets[1] + player_box.dimensions[1]*(y_offset_ratio))
    )

    card_surfaces: list[Surface] = []
    for card in info.cards_in_hand:
        card_surfaces.append(render_card(card))
    hand_frame: FrameTree = compose_surfaces_into_bounding_frame("PlayerHand" + str(player_number), card_surfaces, hand_target_box)

    frame_scaling: float = BoxFunctions.get_scaling_to_fit_first_box_to_second(hand_frame.global_bounding_box, hand_target_box)
    hand_frame.scale_by(frame_scaling, hand_target_box)
    return hand_frame
...
def build_player_frame(background_box: BoundingBox, player_number: int, game_state: GameState, number_of_players: int=2) -> FrameTree:
    ...
    player_frame.add_child(build_player_ui_frame(player_frame.global_bounding_box, player_number, game_state))
    player_frame.add_child(build_player_hand_frame(
        player_frame.global_bounding_box, 
        player_number, 
        game_state, 
        render_hand_topside))
    return player_frame
